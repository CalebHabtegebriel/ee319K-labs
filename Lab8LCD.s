...
BUSY	LDR R1, =SSI0_SR_R		;reading SSIO_SR_R
		LDR R1, [R1]
		AND R2, R1, #0X10		;checking bit 4
		CMP R2, #0
		BNE	BUSY				;loop
		LDR R2, =GPIO_PORTA_DATA_R			;clearing PA6
		LDR R1, [R2]
		AND R1, #0XBF
		STR R1, [R2]
		LDR R1, =SSI0_DR_R		;writing to SSIO_DR_R if bit 4 is low 
		STRB R0, [R1]
BUSY2	LDR R0, =SSI0_SR_R		;reading SSIO_SR_R
		LDR R1, [R0]
		AND R2, R1, #0X10		;checking bit 4
		CMP R2, #0
		BNE	BUSY2				;loop
...
BUSY3	LDR R1, =SSI0_SR_R		;reading SSIO_SR_R
		...
		ORR R1, #0X40
		STR R1, [R2]
		LDR R1, =SSI0_DR_R		;writing to SSIO_DR_R if bit 1 is high 
		STRB R0, [R1]
